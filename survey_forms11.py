    ...
    def _generate_pdf_report(self, report_name, content, report_type, start_date, end_date):
        ...
        try:
            ...
            if content.get('data'):
                ...
                """col_widths = [
                    doc.width * 0.05,  # Job ID
                    doc.width * 0.07,  # Payment ID
                    doc.width * 0.10,  # Title Name
                    doc.width * 0.10,  # Title Number
                    doc.width * 0.18,  # Description
                    doc.width * 0.12,  # Created
                    doc.width * 0.12,  # Payment Date
                    doc.width * 0.08,  # Job Status
                    doc.width * 0.06,  # Payment Status
                    doc.width * 0.06,  # Job Fee
                    doc.width * 0.09,  # Amount Paid
                    doc.width * 0.07,  # Balance
                ]"""

                t = Table(table_data, repeatRows=1) #removed colWidths=col_widths, 
