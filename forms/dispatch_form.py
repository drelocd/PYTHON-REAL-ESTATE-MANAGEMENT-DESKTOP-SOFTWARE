...
class DispatchTitleForm(FormBase):
    """
    A form for dispatching a specific title number.
    """
    def __init__(self, master, db_manager, title_number, refresh_callback, parent_icon_loader):
        super().__init__(master, 500, 450, f"Dispatch Title: {title_number}", "dispatch.png", parent_icon_loader)
        self.db_manager = db_manager
        ...
        # Fetch title details when the form is initialized
        self.title_details = self.db_manager.get_title_details(self.title_number)
    ...
    def _create_widgets(self):
        ...
        title_details_frame = ttk.Frame(main_frame, padding=(0, 0, 0, 20))
        title_details_frame.pack(fill="x")

        # Labels for the information headers
        ttk.Label(title_details_frame, text="Buyer Name:", font=('Helvetica', 10, 'bold')).pack(side="left", padx=10)
        ttk.Label(title_details_frame, text="Location:", font=('Helvetica', 10, 'bold')).pack(side="left", padx=10, expand=True)
        ttk.Label(title_details_frame, text="Title Number:", font=('Helvetica', 10, 'bold')).pack(side="right", padx=10)
        ...
        # Use fetched details to populate the labels
        ttk.Label(data_frame, text=self.title_details.get('buyer_name', 'N/A').upper()).pack(side="left", padx=10)
        ttk.Label(data_frame, text=self.title_details.get('location', 'N/A').upper()).pack(side="left", padx=10, expand=True)
        ttk.Label(data_frame, text=self.title_details.get('title_number', 'N/A').upper()).pack(side="right", padx=10)
    ...
    def _select_files(self):
        ...
        if len(filenames) > 3:
            messagebox.showwarning("File Limit Exceeded", "You can only select a maximum of 3 files.")
            self.selected_files = list(filenames[:3])
        else:
            self.selected_files = list(filenames)
    ...
    def _submit_dispatch(self):
        ...
        if self.db_manager.save_dispatch_details_for_title(
            self.title_number,
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            reason,
            collected_by,
            ...
            messagebox.showinfo("Success", "Title dispatched successfully.")
            self.refresh_callback()
            self.destroy()
        else:
            messagebox.showerror("Error", "Failed to dispatch title.")

class DispatchTitleView(FormBase):
    """
    A form with a notebook to manage dispatching and viewing dispatch records for
    sold properties.
    """
    def __init__(self, master, db_manager, refresh_callback, user_id, parent_icon_loader):
        super().__init__(master, 1200, 600, "Title Dispatch & Records", "dispatch.png", parent_icon_loader)
        self.db_manager = db_manager
        self.refresh_callback = refresh_callback
        self.user_id = user_id
        self.parent_icon_loader = parent_icon_loader
        ...
        self._create_widgets()
        self.populate_sold_properties_table()
        self.populate_dispatch_records_table()
        self._update_dispatch_button_state()
        
    def refresh_all_tables(self):
        """Refreshes both the sold properties and dispatch records tables."""
        self.populate_sold_properties_table()
        self.populate_dispatch_records_table()

    def _create_widgets(self):
        """Creates all the main widgets and tabs for the form."""
        self.notebook = ttk.Notebook(self)
        self.notebook.pack(fill="both", expand=True, padx=10, pady=10)
        
        # Frame for the Dispatch Titles tab
        self.dispatch_titles_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.dispatch_titles_tab, text="Dispatch Titles")
        self._create_dispatch_titles_tab()
    ...
    def _create_dispatch_titles_tab(self):
        """Creates the widgets for the 'Dispatch Titles' tab."""
        search_frame = ttk.Frame(self.dispatch_titles_tab, padding="10 5")
        search_frame.pack(fill="x")
        ttk.Label(search_frame, text="Search:").pack(side="left", padx=(0, 5))
        self.search_entry = ttk.Entry(search_frame)
        self.search_entry.pack(side="left", fill="x", expand=True)
        self.search_entry.bind("<KeyRelease>", self._on_search_change)
        ToolTip(self.search_entry, "Search Sold Properties Using Project Name, Title Number, Location, or Buyer Name.")

        table_frame = ttk.Frame(self.dispatch_titles_tab, padding="10")
        table_frame.pack(fill="both", expand=True)

        columns = ("property_id", "date_sold", "project_name", "title_number", "location", "buyer_name", "status")
        self.sold_properties_tree = ttk.Treeview(table_frame, columns=columns, show="headings")
        
        self.sold_properties_tree.heading("property_id", text="Property ID")
        self.sold_properties_tree.heading("date_sold", text="Date Sold")
        self.sold_properties_tree.heading("project_name", text="Project Name")
        self.sold_properties_tree.heading("title_number", text="Title Number")
        self.sold_properties_tree.heading("location", text="Location")
        self.sold_properties_tree.heading("buyer_name", text="Buyer Name")
        self.sold_properties_tree.heading("status", text="Status")
        
        self.sold_properties_tree.column("property_id", width=60, anchor=tk.CENTER)
        self.sold_properties_tree.column("date_sold", width=120, anchor=tk.W)
        self.sold_properties_tree.column("project_name", width=250, anchor=tk.W)
        self.sold_properties_tree.column("title_number", width=120, anchor=tk.W)
        self.sold_properties_tree.column("location", width=120, anchor=tk.W)
        self.sold_properties_tree.column("buyer_name", width=150, anchor=tk.W)
        self.sold_properties_tree.column("status", width=100, anchor=tk.W)
        
        scrollbar = ttk.Scrollbar(table_frame, orient=tk.VERTICAL, command=self.sold_properties_tree.yview)
        self.sold_properties_tree.configure(yscrollcommand=scrollbar.set)
        
        self.sold_properties_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.sold_properties_tree.bind("<<TreeviewSelect>>", self._update_dispatch_button_state)
        ToolTip(self.sold_properties_tree, "Click to select a sold property to dispatch.")
        
        button_frame = ttk.Frame(self.dispatch_titles_tab, padding="10")
        button_frame.pack(fill="x", side="bottom")
        ...
        self.dispatch_title_btn = ttk.Button(
            button_frame, 
            text="Dispatch Title", 
            image=self.dispatch_icon, 
            compound=tk.LEFT,
        ...
        self.dispatch_title_btn.pack(side="right")
        ToolTip(self.dispatch_title_btn, "Click to open the dispatch form for the selected property.")
    ...
    def _create_dispatch_records_tab(self):
        ...
        columns = (
            "dispatch_id", "dispatch_date", "project_name", "title_number", "collected_by", "collector_phone", "reason_for_dispatch"
        )
        self.dispatch_records_tree = ttk.Treeview(table_frame, columns=columns, show="headings")
        
        self.dispatch_records_tree.column("dispatch_id", width=0, stretch=tk.NO)

        self.dispatch_records_tree.heading("dispatch_date", text="Date")
        self.dispatch_records_tree.heading("project_name", text="Project Name")
        self.dispatch_records_tree.heading("title_number", text="Title Number")
        self.dispatch_records_tree.heading("collected_by", text="Collected By")
        self.dispatch_records_tree.heading("collector_phone", text="Phone Number")
        self.dispatch_records_tree.heading("reason_for_dispatch", text="Reason for Dispatch")
        ...
        self.dispatch_records_tree.column("dispatch_date", width=100, anchor=tk.W)
        self.dispatch_records_tree.column("project_name", width=150, anchor=tk.W)
        self.dispatch_records_tree.column("title_number", width=120, anchor=tk.W)
        self.dispatch_records_tree.column("collected_by", width=150, anchor=tk.W)
        self.dispatch_records_tree.column("collector_phone", width=120, anchor=tk.W)
        self.dispatch_records_tree.column("reason_for_dispatch", width=300, anchor=tk.W)
        ...
        self.dispatch_records_tree.bind("<Double-1>", self._save_signature_file)
        ToolTip(self.dispatch_records_tree, "Double Click a record to download the signed dispatch form.")
    ...
    def populate_sold_properties_table(self):
        """Populates the 'Dispatch Titles' table with sold properties."""
        self.all_sold_properties = self.db_manager.get_sold_properties()
        self._filter_and_populate_table()

    def _filter_and_populate_table(self, search_text=""):
        """Filters the sold properties and populates the table based on search criteria."""
        for item in self.sold_properties_tree.get_children():
            self.sold_properties_tree.delete(item)
        
        lower_search_text = search_text.lower()
        
        for prop in self.all_sold_properties:
            search_data = [
                str(prop.get('property_id', '')).lower(),
                str(prop.get('project_name', '')).lower(),
                str(prop.get('title_number', '')).lower(),
                str(prop.get('location', '')).lower(),
                str(prop.get('buyer_name', '')).lower(),
            ]
            ...
            if any(lower_search_text in s for s in search_data):
                values = (
                    ...
                    prop.get('date_sold', ''),
                    str(prop.get('project_name', '')).upper(),
                    str(prop.get('title_number', '')).upper(),
                    str(prop.get('location', '')).upper(),
                    str(prop.get('buyer_name', '')).upper(),
                    str(prop.get('status', '')).upper()
                )
                self.sold_properties_tree.insert("", tk.END, values=values)
            
    def populate_dispatch_records_table(self):
        """Populates the 'Dispatch Records' table with dispatched titles."""
        self._load_dispatch_records()
        ...
        if records:
            for record in records:
                values = (
                    ...
                    record.get('dispatch_date', ''),
                    str(record.get('project_id', '')).upper(),
                    str(record.get('title_number', '')).upper(),
                    str(record.get('collected_by', '')).upper(),
                    str(record.get('collector_phone', '')).upper(),
                    str(record.get('reason_for_dispatch', '')).upper()
    ...
    def _save_signature_file(self, event):
        ...
        try:
            result = self.db_manager.get_signature_by_dispatch_id(dispatch_id)
            ...
            if result and result.get('sign'):
                ...
                default_filename = f"{project_id}_{title_number}_signature.pdf"
    ...
    def _update_dispatch_button_state(self, event=None):
        """Enables/disables the dispatch button based on row selection."""
        if self.sold_properties_tree.selection():
            self.dispatch_title_btn['state'] = 'normal'
        else:
            self.dispatch_title_btn['state'] = 'disabled'

    def _open_dispatch_details_window(self):
        """Opens the form to enter dispatch details for the selected sold property."""
        selected_item = self.sold_properties_tree.selection()
        if not selected_item:
            messagebox.showinfo("Selection Error", "Please select a sold property to dispatch.")
            return

        # Map column names to their current index
        col_index = {col: idx for idx, col in enumerate(self.sold_properties_tree["columns"])}

        # Get the row values
        row_values = self.sold_properties_tree.item(selected_item[0])['values']

        # Safely fetch the title_number by column name
        title_number = row_values[col_index["title_number"]]

        # Open the Dispatch form with the correct title number
        DispatchTitleForm(
            self,
            self.db_manager,
            title_number,
