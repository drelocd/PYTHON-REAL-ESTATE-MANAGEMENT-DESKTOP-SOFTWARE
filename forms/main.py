...
class RealEstateApp(tk.Tk):
    ...
    def check_for_updates(self):
        ...
        # Pass auto_install=True to trigger silent download and restart prompt
        threading.Thread(target=self._run_update_check, args=(True,), daemon=True).start()

    def _run_update_check(self, auto_install=False):
        """
        Performs the actual update check by fetching release information from GitHub.
        This method runs in a separate thread.
        `auto_install`: If True, the update will download automatically without user prompt,
                        and then prompt for a restart to install.
        """
        ...
        try:
            ...
            for asset in assets:
                if asset["name"].lower().endswith(".exe"):  # Assuming a Windows executable
                    download_url = asset["browser_download_url"]
                    asset_filename = asset["name"]
                    break

            if not download_url:
                # Fallback to the HTML URL if no executable is found
                download_url = release_info.get("html_url")
                asset_filename = f"MathengeRealEstateApp-Update-{latest_version_tag}.zip" # Assuming a zip for source
                ...
                if self._is_newer_version(latest_version_clean):
                    ...
                    if auto_install:
                        self.after(0, lambda lv=latest_version_clean, dl=download_url,
                                             af=asset_filename: self._automatic_download_and_prompt_restart(lv, dl, af))
                    else:
                        self.after(0, lambda lv=latest_version_clean, dl=download_url,
                    ...
                    if not auto_install: # Only show this message if not in silent mode
    ...
    def _prompt_and_download_update(self, latest_version, download_url, asset_filename):
        """
        (Original method) Prompts the user if they want to download the update and then initiates the download.
        """
        if messagebox.askyesno(
            ...
            threading.Thread(target=self._download_update, args=(download_url, asset_filename, latest_version, False),
                             daemon=True).start()
        else:
    ...
    def _automatic_download_and_prompt_restart(self, latest_version, download_url, asset_filename):
        """
        Automatically downloads the update and then prompts the user to restart for installation.
        This replaces the _prompt_and_download_update logic for silent updates.
        """
        self.logger.info(f"Automatically downloading update to version {latest_version}.")
        threading.Thread(target=self._download_update, args=(download_url, asset_filename, latest_version, True),
                         daemon=True).start()

    def _download_update(self, url, filename, latest_version, is_silent_download=False):
        """
        Downloads the update file to a temporary directory.
        `is_silent_download`: If True, will show a different, less intrusive message.
        """
        ...
        try:
            if not is_silent_download:
                ...
                self.after(0, lambda: messagebox.showinfo("System Update",
                                                          f"A new version is downloading in the background. You will be prompted to restart once it's ready!"))
            ...
            if is_silent_download:
                self.after(0, lambda: messagebox.askyesno(
                    ...
                    "**Please restart the application now to complete the update.**\n"
                    "Would you like to close the application to restart now?"
                ) and self.after(0, self.on_exit)) # If yes, close the app
            else:
    ...
    def on_login_complete(self, success, user_type=None, user_id=None):
        ...
        if self.login_successful:
            ...
            # NEW: Check for updates right after successful login, now with auto_install=True by default for silent updates
    ...
    def _create_menu_bar(self):
        ...
        if self.user_type in ['admin', 'accountant', 'field_worker']: # Added field_worker to survey menu access
            ...
            surveys_menu.add_command(label="Track Jobs",
                                     ...
                                     state='normal' if self.user_type in ['admin', 'field_worker'] else 'disabled')  # Admin and Field Worker can track jobs
            surveys_menu.add_command(label="Manage Payments",  # Added this menu item
                                     command=lambda: self.survey_section._open_manage_payments_view(), # Corrected method call
            ...
            surveys_menu.add_command(label="Survey Reports",  # Added this menu item
                                     command=lambda: self.survey_section._open_job_reports_view(), # Corrected method call
                                     state='normal' if self.user_type in ['admin', 'accountant'] else 'disabled')
        ...
        reports_menu.add_command(label="Pending Instalments Report",
                                 ...
                                 state='normal' if self.user_type in ['admin', 'accountant'] else 'disabled')
        # Corrected method calls for survey reports
        ...
        # Now this will trigger the manual check, with a user prompt
        help_menu.add_command(label="Check for Updates...", command=lambda: self._run_update_check(auto_install=False))
    ...
    def _go_to_survey_tab_and_action(self, action):
        ...
        if action == "add_job":
            self.survey_section._open_add_client_and_file_form() # This form handles adding clients and files (jobs)
