...
if __name__ == '__main__':
    ...
    class MockApp(tk.Tk):
        def __init__(self):
            ...
            self.admin_user_id = self.db_manager.get_user_by_username("test_admin")['user_id']

            # Add some dummy logs for testing
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "USER_ADDED", "User 'new_user' added.")
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "PROPERTY_ADDED",
                                             "Property 'ABC-123' added.")
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "SETTING_UPDATED", "Tax rate changed.")
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "PROPERTY_SOLD",
                                             "Property 'XYZ-789' sold.")
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "SURVEY_JOB_ADDED",
                                             "New survey job created for client John Doe.")
            self.db_manager.add_activity_log(self.admin_user_id, "test_admin", "USER_UPDATED",
                                             "User 'test_admin' password changed.")

            self.open_logs_button = ttk.Button(self, text="Open Activity Logs", command=self.open_logs)
            self.open_logs_button.pack(pady=50)
            self.deiconify()  # Show main window after setup

        def open_logs(self):
            logs_form = ActivityLogViewerForm(self, self.db_manager, parent_icon_loader=self._mock_load_icon)
            logs_form.wait_window()
