...
class DatabaseManager:
    ...
    def _execute_query(self, query, params=(), fetch_one=False, fetch_all=False):
        ...
        try:
            with conn.cursor(dictionary=True) as cursor: # Use dictionary=True for dictionary-like access
    ...
    def add_service_client(self, name, telephone_number, email, brought_by, added_by):
        ...
        try:
            cursor.execute(''' INSERT INTO service_clients (name, telephone_number, email, brought_by, added_by, timestamp) VALUES (%s, %s, %s, %s, %s, %s) ''', (name, telephone_number, email, brought_by, added_by, timestamp))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.IntegrityError:
            return None
    ...
    def add_client_file(self, client_id, file_name, added_by):
        ...
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        try:
            cursor.execute(''' INSERT INTO client_files (client_id, file_name, added_by, timestamp) VALUES (%s, %s, %s, %s) ''', (client_id, file_name, added_by, timestamp))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.IntegrityError:
            return None
    ...
    def add_service_job(self, file_id, job_description, title_name, title_number, fee, status, added_by, brought_by):
        ...
        try:
            ...
            ''', (file_id, job_description, title_name, title_number, fee, status, added_by, brought_by, timestamp))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.IntegrityError:
            return None
    ...
    def add_service_payment(self, job_id, fee, amount, balance, payment_date):
        ...
        try:
            cursor.execute('''
                ...
                VALUES (%s, %s, %s, %s, %s)
            ''', (job_id, fee, amount, balance, payment_date))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.IntegrityError:
            return None
    ...
    def add_payment_history(self, payment_id, payment_amount, payment_type):
        ...
        try:
            cursor.execute('''
                ...
                VALUES (%s, %s, %s, %s)
            ''', (payment_id, payment_amount, payment_type, payment_date))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.Error as e:
    ...
    def add_service_dispatch_record(self, job_id, reason_for_dispatch, collected_by, collector_phone, sign):
        ...
        try:
            cursor.execute('''
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            ''', (job_id, dispatch_date, reason_for_dispatch, collected_by, collector_phone, sign))
            conn.commit()
            return cursor.lastrowid
        except mysql.connector.Error as e:
            print(f"Error adding dispatch record: {e}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def add_activity_log(self, user_id, action_type, details=None):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            cursor.execute("INSERT INTO activity_logs (timestamp, user_id, action_type, details) VALUES (%s, %s, %s, %s)",
